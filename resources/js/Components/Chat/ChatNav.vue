<script setup>
    ...
    const goToChat = (id) =>{
        emit('navChat', {chat: true, user_id: id})
    }
...
<template>
    <div class="flex flex-col h-96">
        ...
        <div id="userChatList" class="flex w-full mt-2 flex-col overflow-auto" ref="userChatList" v-on:scroll="onScroll($event)">
            <button v-for="user in users" type="button" class="w-full hover:bg-indigo-700 hover:text-white rounded p-2" @click="goToChat(user.id)">
